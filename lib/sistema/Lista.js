...
function nuevoLista() {
    ...
    btnNuevaLista.addEventListener('click', () => {
        ...
        let dirigentesLimpiar = document.querySelector('#selectDirigente');

        while (dirigentesLimpiar.firstChild) {
            dirigentesLimpiar.removeChild(dirigentesLimpiar.firstChild);
        }
        ...
        let dirigentes = cargaDirigentes().then((result) => {
            //console.log("el punteros limpiar ", punterosLimpiar);
            //            console.log('movimientos then', result);
            result.forEach(element => {
                console.log('element', element);
                let option = document.createElement('option');
                option.value = element.id_dirigente;
                option.textContent = element.nombre;
                document.getElementById('selectDirigente').appendChild(option);
            });
