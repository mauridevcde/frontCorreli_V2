...
import { headerPersona, formularioPersona, modalGrupos, spinnerDeCargaPersona } from '../views/TemplatePersona.js'
...
async function inicioPersona() {
    ...
    view.spinnerDeCargaPersona = await spinnerDeCargaPersona()
    ...
    persona_Check_Post_Put(view)
    ocultarTodoInputs()
}

let imagenBase64ParaElPut;
async function persona_Check_Post_Put(view) {
        ...
        if (ci.value == "" || ci.value == "0" || ci.value == null || ci.value == undefined || ci.value == " ") {
            ...
            if (VerificadorCi == 1) {
                ...
                Swal.fire({
                   ...
                   //Spinner de carga por persona
                   ocultarBtnEditar()
                   ocultarTodoInputs()
                    window.paraElEspinner.innerHTML = ` <span class="spinner-border spinner-border-bg "id="relogRender1" style="color:white" role="status" aria-hidden="true"></span>`
    ...
    window.btnEditarNuevaLista.addEventListener('click', async function (e) {
        ...
        console.log('validador de existencia Ci', validadorExistenciaCiEnListas.result[0].result)

        if (validadorExistenciaCiEnListas.result[0].result == '1') {
            Swal.fire({
                icon: 'error',
            ...
            if (localStorage.getItem('id_grupo_AddLista') === 'noExiste') {

                Swal.fire({
                    icon: 'error',
                    title: "El Puntero NO Tiene grupo asignado.",
                    text: 'Favor pedir a un Dirigente para asignar un grupo.',
                    confirmButtonText: "OK",
                    confirmButtonColor: "#6F8EAD",
                })
            } else {
                ...
                objGrupos.forEach(element => { //llama a la api provincias pero usa su objeto provinciaPy para mostrar 
                    ...
                    grupoRender += `<option value="${element.id_grupo}" >${element.nombre}</option>`
        ...
        window.btnGuardarNuevoGrupo2.addEventListener('click', async function (e) {
            let modal2 = bootstrap.Modal.getOrCreateInstance(document.getElementById('modalSeleccionarGrupo'))
