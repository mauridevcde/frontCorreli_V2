...
async function BuscarColo_o_PorCi(view, model ) {
    ...
    async function buscarPorCedula() {
        ...
        let btnNuevoCorreli = document.getElementById('btnNewCorreliNuevo');
        btnGuardar.style.display = 'none';
        btnEditar.style.display = 'none'
        btnNuevaPersona.style.display = 'none';
        btnEditarNewCorreli.style.display = 'none';
        btnNuevoCorreli.style.display = 'none';


        let btnEnviarListaVotantes = document.getElementById('btnEnviarListaVotantes')
           
        btnEnviarListaVotantes.addEventListener('click', async () => {
            Swal.fire({
                icon: 'success',
                title: "Enviado a Votantes",
            ...
            if (New_Persona.length > 0) {
                ...
                if (New_Correli.length > 0) {
                    ...
                    btnEditarNewCorreli.addEventListener('click', function () {
                        ...
                        let nombreOldComparacion = Old_Correli[0].nombre;
                        let apellidoOldComparacion = Old_Correli[0].apellido;
                        let c_fenaciOldComparacion = Old_Correli[0].c_fenaci;
                        let n_partidoOldComparacion = Old_Correli[0].n_partido;
                        let direccionOldComparacion = Old_Correli[0].direccion;
                        let departOldComparacion = Old_Correli[0].depart;
                        let distritoOldComparacion = Old_Correli[0].distrito;
                        let localidadOldComparacion = Old_Correli[0].localidad;
                        let mesaOldComparacion = Old_Correli[0].mesa;
                        let ordenOldComparacion = Old_Correli[0].orden;
                        let zonaOldComparacion = Old_Correli[0].zona;

                        let Objeto_Old_Correli = {
                            nombre: nombreOldComparacion.toString(),
                            apellido: apellidoOldComparacion.toString(),
                            fechaNacimiento: c_fenaciOldComparacion.toString(),
                            partido: n_partidoOldComparacion.toString(),
                            direccion: direccionOldComparacion.toString(),
                            departamento: departOldComparacion.toString(),
                            distrito: distritoOldComparacion.toString(),
                            localidad: localidadOldComparacion.toString(),
                            mesa: mesaOldComparacion.toString(),
                            orden: ordenOldComparacion.toString(),
                            zona: zonaOldComparacion.toString()
                        }

                        let n_cedulaComparacion = document.getElementById('searchCiColorado').value;
                        let nombreOldCorreliComparacion = document.getElementById('inputName').value;
                        let apellidoOldCorreliComparacion = document.getElementById('inputSurname').value;
                        let fechaNacimientoOldCorreliComparacion = document.getElementById('inputDate').value;
                        let partidoOldCorreliComparacion = document.getElementById('inputPartido').value;
                        let direccionOldCorreliComparacion = document.getElementById('inputDireccion').value;
                        let departamentoOldCorreliComparacion = document.getElementById('inputDepartamento').value;
                        let distritoOldCorreliComparacion = document.getElementById('inputDistrito').value;
                        let localidadOldCorreliComparacion = document.getElementById('inputLocalidad').value;
                        let mesaOldCorreliComparacion = document.getElementById('inputMesa').value;
                        let ordenOldCorreliComparacion = document.getElementById('inputOrden').value;
                        let zonaOldCorreliComparacion = document.getElementById('inputZona').value;

                        let Objeto_Old_Correli2 = {
                            nombre: nombreOldCorreliComparacion.toString(),
                            apellido: apellidoOldCorreliComparacion.toString(),
                            fechaNacimiento: fechaNacimientoOldCorreliComparacion.toString(),
                            partido: partidoOldCorreliComparacion.toString(),
                            direccion: direccionOldCorreliComparacion.toString(),
                            departamento: departamentoOldCorreliComparacion.toString(),
                            distrito: distritoOldCorreliComparacion.toString(),
                            localidad: localidadOldCorreliComparacion.toString(),
                            mesa: mesaOldCorreliComparacion.toString(),
                            orden: ordenOldCorreliComparacion.toString(),
                            zona: zonaOldCorreliComparacion.toString()
                        }

                        // compara si los objetos son iguales: si son iguales no se hace nada, si no son iguales se hace una actualizacion
                        //comparar el objeto1:  Objeto_Old_Correli con el objeto2: Objeto_Old_Correli2
                        let comparacionOldCorreli = ObjCompare(Objeto_Old_Correli, Objeto_Old_Correli2);

                        let nuevaPersona2 = {

                            n_cedula: n_cedulaComparacion,
                            nombre: nombreOldCorreliComparacion,
                            apellido: apellidoOldCorreliComparacion,
                            telefono: telefonoComparacion,
                            email: emailComparacion,
                            distrito: distritoOldCorreliComparacion,
                            foto: "0",
                            partido: partidoOldCorreliComparacion,
                            latitud: latitudComparacion,
                            longitud: longitudComparacion,
                            observaciones: observacionesComparacion
                        ...
                        if (comparacionOldCorreli == true) {
                            console.log('son iguales old correli');

                        } else {
                            console.log('no son iguales old correli');

                            if (validaObject(nuevaPersona2)) {
                                console.log("true");
                                post_nuevaPersona(nuevaPersona2).then((res) => {
                                    if (res.message == 'ok') {
                                        Swal.fire({
                                            icon: 'success',
                                ...
                                console.log('dentro del validador de objetos');
                            } else {
                        ...
                        let comparacionNewCorreli = ObjCompare(Objeto_New_Correli, Objeto_New_Correli2);


                        if (comparacionNewCorreli == true) {
                            console.log('son iguales NEW CORRELI');
                        } else {
                            console.log('no son iguales NEW CORRELI');
                            if (validaObject(Objeto_New_Correli2)) {
                                console.log('dentro del validador de objetos new correli 2');
                    ...
                    if (Old_Correli.length > 0) {
                        ...
                        btnNuevoCorreli.style.display = 'grid';
                        ...
                        btnNuevoCorreli.addEventListener('click', () => {

                            console.log('click boton nuevo correli');

                            let latitudNuevoCorreli = document.getElementById('contactLat').value
                            let longitudNuevoCorreli = document.getElementById('contactLng').value
                            let emailNuevoCorreli = document.getElementById('inputCorreo').value
                            let observacionesNuevoCorreli = document.getElementById('textAreaObs').value
                            let cedulaNuevoCorreli = document.getElementById('searchCiColorado').value
                            let telefonoNuevoCorreli = document.getElementById('inputTelefonoPersonas').value

                            let nuevoCorreli33 = {
                                latitud: latitudNuevoCorreli,
                                longitud: longitudNuevoCorreli,
                                email: emailNuevoCorreli,
                                foto: '0',
                                observaciones: observacionesNuevoCorreli,
                                n_cedula: cedulaNuevoCorreli,
                                telefono: telefonoNuevoCorreli
                            }

                            console.log("🚀 ~ file: Persona.js ~ line 50 ~ nuevoCorreli33", nuevoCorreli33);
                            if (validaObject2(nuevoCorreli33)) {
                                console.log("true");

                                post_newCorreli(nuevoCorreli33).then((res) => {
                                    if (res.message == 'ok') {
                                        Swal.fire({
                                            icon: 'success',
                                            title: "Nueva Persona Agregada!",
...
async function post_newCorreli(datos) {
    let newCorreli = await api_post_new_Correli(datos);
    return newCorreli;
}
