...
export function tablaPuntero(props) {
    ...
    if (!localStorage.getItem('id_dirigente')) {
        ...
        punteroPorDirigente.forEach((element) => {
            retorno += /*html*/ `
              <tr>
                <td data-id="#: "> ${element.grupos.punteros_id_puntero} </td>
                <td data-id="NOMBRE: "> ${element.punteros.nombre} </td>
                <td data-id="DESCR: "> ${element.punteros.telefono} </td>
             <td class="d-flex justify-content-center"> 
                 <button style="margin-right: 5px;" id="${element.grupos.punteros_id_puntero}" 0 data-id = 0 class="btn btn-info btnNewViewEditPuntero mr-2 " title="Ver Puntero"><i class="fa fa-eye" aria-hidden="true" style="font-size: 20px;"></i></button>
                 <button id="${element.grupos.punteros_id_puntero}" data-id = 0 class="btn btn-danger btnEliminarPuntero mr-2 " title="Eliminar Puntero"><i class="fa fa-trash-o" aria-hidden="true" style="font-size: 20px;"></i></button>
             </td>
              </tr>`;
